...
export default function CustomerMemoryManager() {
  const [page, setPage] = useState<MemoryPage>('manage');
      ...
      {page === 'config' && (
        <div className="space-y-4">
          <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm flex items-center justify-between">
            <div><h2 className="text-xl font-bold text-slate-900">记忆配置</h2><p className="text-sm text-slate-500 mt-1">配置记忆开关、自定义字段、抽取提示词和通话小结提示词。</p></div>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
            <h3 className="font-bold text-slate-900 flex items-center gap-2"><Database size={18} />自定义字段配置</h3>
            <div className="mt-4 grid grid-cols-5 gap-3 text-sm">{fieldRecords.map(field => <div key={field.code} className="border border-slate-100 rounded-xl p-3"><div className="font-semibold">{field.name}</div><div className="text-xs text-slate-400 font-mono mt-1">{field.code}</div><div className="text-xs text-slate-500 mt-2">有效期：{field.ttl}</div></div>)}</div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-slate-900 text-blue-50 rounded-2xl p-5 text-sm leading-7 whitespace-pre-wrap"><FileText size={18} className="mb-3" />你需要从本轮通话文本、工具返回结果和已有客户上下文中抽取配置字段。只抽取明确出现的信息，不允许猜测。</div>
            <div className="bg-slate-900 text-blue-50 rounded-2xl p-5 text-sm leading-7 whitespace-pre-wrap"><FileText size={18} className="mb-3" />你需要为本轮语音通话生成一条可供下一轮语音 Agent 使用的小结，小结长度控制在 120 字以内。</div>
          </div>
        </div>
      )}
