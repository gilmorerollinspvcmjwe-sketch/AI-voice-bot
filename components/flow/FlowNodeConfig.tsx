        ...
        {localNode.type !== FlowNodeType.START ? (
          <Section title="提示词" icon={<FileText size={16} />}>
            <PromptEditor
              ...
              height="h-32"
              placeholder="描述这个步骤要完成的对话目标、工具使用方式和限制。"
            />
            ...
            {isStepNode ? (
              <div className="space-y-3 border-t border-slate-100 pt-3">
                {delayProfiles.length ? (
                  <div>
                    <div className="mb-2 text-xs font-medium text-slate-600">延迟话术</div>
                    <div className="flex flex-wrap gap-2">
                      {delayProfiles.map((profile) => {
                        const selected = localNode.data.delayProfileId === profile.id;
                        return (
                          <button
                            key={profile.id}
                            type="button"
                            disabled={readOnly}
                            onClick={() => updateNodeData({ delayProfileId: selected ? '' : profile.id })}
                            className={`rounded-full border px-3 py-1.5 text-xs ${selected ? 'border-transparent bg-emerald-600 text-white' : 'border-emerald-200 bg-white text-slate-600'}`}
                          >
                            {profile.name}
                          </button>
                        );
                ...
                <div>
                  <div className="mb-2 text-xs font-medium text-slate-600">工具</div>
                  <div className="flex flex-wrap gap-2">
                    {availableTools.map((tool) => {
                      const selected = (localNode.data.toolIds || []).includes(tool.id);
                      return (
                        <button key={tool.id} type="button" disabled={readOnly} onClick={() => updateNodeData({ toolIds: toggleStringId(localNode.data.toolIds, tool.id) })} className={`rounded-full border px-3 py-1.5 text-xs ${selected ? 'border-transparent bg-slate-900 text-white' : 'border-slate-200 bg-white text-slate-600'}`}>
                          {tool.name}
                        </button>
                      );
                    })}
                ...
                <div>
                  <div className="mb-2 text-xs font-medium text-slate-600">代码块</div>
                  <div className="flex flex-wrap gap-2">
                    {codeBlocks.map((item) => {
                      const selected = (localNode.data.codeBlockIds || []).includes(item.id);
                      return (
                        <button key={item.id} type="button" disabled={readOnly} onClick={() => {
                          const nextIds = toggleStringId(localNode.data.codeBlockIds, item.id);
                          updateNodeData({ codeBlockIds: nextIds });
                          updateStepPrompt({ codeBlockIds: nextIds });
                        }} className={`rounded-full border px-3 py-1.5 text-xs ${selected ? 'border-transparent bg-indigo-700 text-white' : 'border-indigo-200 bg-white text-slate-600'}`}>
                          {item.name}
                        </button>
                      );
                    })}
          ...
          <Section title="高级设置" icon={<Sparkles size={16} />} defaultExpanded={false}>
            ...
            {localNode.data.retryConfig?.enabled ? (
              <>
                ...
                <div><Label label="确认话术" /><textarea rows={2} value={localNode.data.retryConfig?.confirmationPrompt || ''} disabled={readOnly} onChange={(e) => updateNodeData({ retryConfig: { ...localNode.data.retryConfig, enabled: true, confirmationPrompt: e.target.value } })} className="w-full resize-none rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/10 disabled:bg-slate-100" /></div>
                <div className="grid grid-cols-2 gap-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div><Label label="兜底 Flow" /><input value={localNode.data.retryConfig?.fallbackFlowId || ''} disabled={readOnly} onChange={(e) => updateNodeData({ retryConfig: { ...localNode.data.retryConfig, enabled: true, fallbackFlowId: e.target.value } })} className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/10 disabled:bg-slate-100" /></div>
                  <div><Label label="转人工目标" /><select value={localNode.data.retryConfig?.handoffTargetId || ''} disabled={readOnly} onChange={(e) => updateNodeData({ retryConfig: { ...localNode.data.retryConfig, enabled: true, handoffTargetId: e.target.value } })} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/10 disabled:bg-slate-100"><option value="">未指定</option>{HANDOFF_OPTIONS.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></div>
                </div>
              </>
            ) : null}
            <div className="border-t border-dashed border-slate-200 pt-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><Label label="目标 Flow" /><input value={localNode.data.gotoFlowId || ''} disabled={readOnly} onChange={(e) => updateNodeData({ gotoFlowId: e.target.value })} className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/10 disabled:bg-slate-100" placeholder="用于子 Flow 跳转或 return flow" /></div>
          ...
          <Section title="退出行为" icon={<LogOut size={16} />}>
            ...
            {(localNode.data.exitType || ExitNodeType.FINISH) === ExitNodeType.HANDOFF ? (
              <div className="space-y-3">
                ...
                {/* 需求 2：转人工摘要模板 */}
                <div>
                  <Label label="转人工摘要模板" tooltip="转人工时生成的会话摘要模板，支持变量引用" />
                  <textarea 
                    rows={4} 
                    value={localNode.data.handoffSummaryTemplate || ''} 
                    disabled={readOnly} 
                    onChange={(e) => updateNodeData({ handoffSummaryTemplate: e.target.value })} 
                    ...
                    placeholder="用户 {{user_name}} 需要转人工处理，原因：{{reason}}，当前状态：{{state_summary}}"
                  />
                </div>
              </div>
            ) : null}
